import os
import pdb
from .data_ner import BertNERDataBunch, InputExample, InputFeatures
...
class BertNERLearner(object):
    ...
    def from_pretrained_model(dataBunch, pretrained_path, output_dir, metrics, device, logger, finetuned_wgts_path=None, 
        ...
        model = model_class.from_pretrained(pretrained_path, config=config, state_dict=model_state_dict)
        
        device_id = torch.cuda.current_device() 
        model.to(device)
    
            
        return BertNERLearner(dataBunch, model, pretrained_path, output_dir, metrics, device, logger,
    ...
    def __init__(self, data: BertNERDataBunch, model: nn.Module, pretrained_model_path, output_dir, metrics, device,logger,
                 multi_gpu=True, is_fp16=True, loss_scale=0, warmup_steps=0, fp16_opt_level='O1',
    ...
    def validate(self):
        ...
        for step, batch in enumerate(progress_bar(self.data.val_dl)):
            ...
            #logits = self.get_active_logits(logits, inputs['attention_mask'])
            #pdb.set_trace()
            ...
            if preds is None:
                preds = np.argmax(logits.detach().cpu().numpy(), 2)
                out_label_ids = inputs['labels'].detach().cpu().numpy()
            else:
                preds = np.append(preds, np.argmax(logits.detach().cpu().numpy(), 2), axis=0)
                out_label_ids = np.append(out_label_ids, inputs['labels'].detach().cpu().numpy(), axis=0)
        
        eval_loss = eval_loss / nb_eval_steps
        
        # Evaluation metrics - classification report
        sep_label_id = self.data.labels.index('[SEP]')
        y_true = []
        y_pred = []
        for i, label in enumerate(out_label_ids):
            ...
            for j,m in enumerate(label):
                if j == 0:
                    continue
                elif out_label_ids[i][j] == sep_label_id:
                    y_true.append(temp_1)
                    y_pred.append(temp_2)
                    break
                else:
                    temp_1.append(self.data.label_map[out_label_ids[i][j]])
                    temp_2.append(self.data.label_map[preds[i][j]])
    ...
    def get_active_logits(self, logits, attention_mask):
        # get the argmax for each token
        active_logits = torch.max(logits, 2)[1]
        
        # apply attention mask to ignore masked logits
        active_logits = active_logits * attention_mask
    ...
    def predict_batch(self,texts):
        ...
        dl,_ = self.data.get_dl_from_texts(texts)
        ...
        logits_label = np.argmax(all_logits,2)
        preds = []
        for i, logit in enumerate(logits_label):
            # get valid subset

            words = texts[i].split(' ')
            logit = logit[1:len(words)+1]
            logit_labels = [self.data.label_map[i] for i in logit]

            logits_confidence = [values[label].item() for values,label in zip(all_logits[i][1:len(words)+1],logit)]

            pred = [{'word': word, 
                     'label': label,
                    'confidence': confidence} for word, label, confidence in zip(words, logit_labels, logits_confidence)]
            preds.append(pred)
